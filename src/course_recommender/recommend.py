...
пререквизиты, приоритеты регистрации и то, как курс заполнялся в прошлые семестры.
...
досталось место. Поэтому шанс считается на уровне курса и его приоритета,
...
# какого приоритета, и очередь по ним не восстанавливается ни одной моделью. Пока
...
class Evidence:
    ...
    def seat_chance(self) -> float:
        ...
        достанется ли место именно этому приоритету. Первое измеримо и обучено на
        ...
        Это оценка уровня курса и приоритета, а не персональная вероятность:
...
def recommend(
    ...
    offerings — курсы каталога (приоритеты регистрации), term — семестр регистрации,
    за который эти приоритеты брать, fill_history — заполняемость прошлых семестров,
