...
* настоящие пререквизиты — с логикой AND/OR, скобками и порогом по каждому
  ...
  на C-;
* очереди приоритета регистрации — кому место дают в первую очередь, кому во
...
# Школы переименовывались и делились, а документы прошлых семестров знают
# только прежние названия — и выданный тогда приоритет относится к студентам
# сегодняшних школ. SEDS разошлась на SCAI (вычисления, математика) и SoE
# (инженерия); кафедры горного дела и геонаук ушли из SMG в SoE, в SMG
# осталась нефтянка.
#
# Связь односторонняя: приоритет, выданный сегодня SoE, на студента SCAI не
# распространяется. Без этих правил студент CS в документе Spring 2025 имел
# бы приоритет на 18 курсах вместо 231, геолог — на 211 вместо 217.
SCHOOL_SUCCESSORS = {
    "SEDS": frozenset({"SCAI", "SOE"}),
    "SMG": frozenset({"SOE"}),
}
